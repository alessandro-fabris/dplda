...
%       I MODIFUED THE CODE SO I HAVE ACCESS TO LOGLIKE MATCH AND LOGLIKE
%       NO-MATCH SEPARATELY
...
% 04-03-2010
%
function [logLikeMatch,logLikeNoMatch] = PLDA_Verification_optimized(Model, vid1, vid2)
...
vid1=vid1-repmat(meanVec,1,T);
vid2=vid2-repmat(meanVec,1,T);
...
Data1P = preProcessPLDAData(factorModel, vid1);
Data2P = preProcessPLDAData(factorModel, vid2);

logLikeNoMatch=zeros(T);
logLikeMatch=zeros(T);
marginalLoglike=zeros(2,T);
%  disp('computing marginal1')
for t1=1:T
    marginalLoglike(1,t1)=getLogLikeMatchPLDA(factorModel, Data1P(t1));
end

%  disp('computing marginal2')
for t2=1:T
    marginalLoglike(2,t2)=getLogLikeMatchPLDA(factorModel, Data2P(t2));
end

%  disp('computing joint')

for t1=1:T
   for t2=1:T
       logLikeMatch(t1,t2) = getLogLikeMatchPLDA(factorModel, [Data1P(t1), Data2P(t2)]);
       logLikeNoMatch(t1,t2)=marginalLoglike(1,t1)+marginalLoglike(2,t2);
   end
end
