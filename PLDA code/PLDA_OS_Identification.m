...
%**************************************************************************
% Date: 04-03-2010
function LogLikelihood = PLDA_OS_Identification(Model, DataGallery, DataProbe)
...
LogLikelihood = zeros(nGallery+1, nProbe);
...
%             - logLikeGalleryNoMatch(cGallery);
        ...
        logLikeModel = getLogLikeMatchPLDA(factorModel, ...
            [galleryDataPP(cGallery) probeDataPP(cProbe)])- logLikeGalleryNoMatch(cGallery);
        LogLikelihood(cGallery, cProbe) = logLikeModel; %loglik of cProb conditional on cGal
    end
end
loglik_marg=zeros(1,nProbe);
parfor cProb=1:nProbe
    loglik_marg(cProb)=getLogLikeMatchPLDA(...
        factorModel, probeDataPP(cProb));
end
LogLikelihood(nGallery+1, :)=loglik_marg;
